...
type ColumnType = ColumnDef<TodoInterface, unknown>;
...
export function TodoListPage(props: TodoListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getTodos({
        relations: ['organization', 'user', 'reroll.count'],
        limit: params.pageSize,
        offset: params.pageNumber * params.pageSize,
        searchTerm: params.searchTerm,
        order: params.order,
        searchTermKeys: ['task.contains', 'hyperlink.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<TodoInterface>>(
    () => `/todos?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: TodoInterface) => {
    if (hasAccess('todo', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/todos/view/${row.id}`);
    }
  };

  const columns: ColumnType[] = [
    { id: 'task', header: 'Task', accessorKey: 'task' },
    { id: 'hyperlink', header: 'Hyperlink', accessorKey: 'hyperlink' },
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('reroll', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'reroll',
          header: 'Reroll',
          accessorKey: 'reroll',
          cell: ({ row: { original: record } }: any) => record?._count?.reroll,
        }
      : null,

    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/todos/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"

...
import { getRerollById } from 'apiSdk/rerolls';
import { RerollInterface } from 'interfaces/reroll';

function RerollViewPage() {
        ...
        {isLoading ? (
          ...
          <>
            <FormWrapper wrapperProps={{ border: 'none', gap: 3, p: 0 }}>
              ...
              >
                <FormListItem label="Reroll Count" text={data?.reroll_count} />
                ...
                {hasAccess('todo', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                  <FormListItem
                    label="Todo"
                    text={
                      <Link as={NextLink} href={`/todos/view/${data?.todo?.id}`}>
                        {data?.todo?.task}
                      </Link>
                    }
                  />
